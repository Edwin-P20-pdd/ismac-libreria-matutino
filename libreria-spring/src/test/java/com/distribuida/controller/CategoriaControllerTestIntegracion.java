...
public class CategoriaControllerTestIntegracion {
    ...
    public void testFindAll() throws Exception {
        Categoria categoria = new Categoria(1, "Ficción", "Libros de narrativa imaginativa");

        Mockito.when(categoriaService.findAll()).thenReturn(List.of(categoria));

        mockMvc.perform(get("/categorias"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].categoria").value("Ficción"));
    }

    @Test
    public void testSave() throws Exception {
        Categoria categoria = new Categoria(0, "Ficción", "Libros de narrativa imaginativa");
        ...
        mockMvc.perform(post("/categorias")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.categoria").value("Ficción"));
    }
